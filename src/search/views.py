...
def index(request: HttpRequest) -> HttpResponse:
    ...
    if query:
        logger.info(
            "Search request received: query='%s', user=%s, IP=%s",
            query,
            user_identifier,
        ...
        if url:
            logger.info(
                "Bang resolved for query '%s' (user: %s) -> redirecting to: %s",
                query,
                user_identifier,
                url,
            )
        ...
        if query_without_bang:
            logger.info(
                "Bang query '%s' resolved to search query '%s' (user: %s)",
                query,
                query_without_bang,
                user_identifier,
            )
        ...
        logger.info(
            "Executing meta-search for query: '%s' (user: %s)",
            search_query,
            user_identifier,
        )
        ...
        if not results:
            ...
            logger.warning(
                "No results found for query '%s' (user: %s) - "
        ...
        logger.info(
            "Successfully found %d results for query '%s' (user: %s)",
            len(results),
            query,
            user_identifier,
        )
        ...
        logger.debug(
            "Empty search request (user: %s, IP: %s)",
            user_identifier,
            request.META.get("REMOTE_ADDR", "unknown"),
        )
