...
result=$(nix build .#packages.aarch64-linux.nginx-image --no-link --print-out-paths)
...
manifest-tool --username $REGISTRY_USER --password $REGISTRY_PASS push from-args --platforms linux/arm64/v8 --template "${REGISTRY,,}"/zweili-search-nginx:latest --target "${REGISTRY,,}"/zweili-search-nginx:latest
