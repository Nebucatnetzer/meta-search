...
result=$(nix build .#packages.x86_64-linux.app-image --no-link --print-out-paths)
...
manifest-tool --username $REGISTRY_USER --password $REGISTRY_PASS push from-args --platforms linux/amd64 --template "${REGISTRY,,}"/zweili-search-app:latest --target "${REGISTRY,,}"/zweili-search-app:latest
